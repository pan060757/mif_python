...
#####住院数据预处理
###（个人编号 1,(就医序号 2，医疗机构代码 3,医院等级 5，住院天数，药品费 10,起付线 14,报销比例 15，统筹账户支付 17,入院日期 21,出院日期 22，病种 26)))）
def hospitalProcessing(line):
    ...
    try:
        ...
        if line[27] == '腰椎间盘突出'.decode("utf-8"):
            if (line[5] == '无等级'.decode("utf-8")):
                line[5] = '0'
            elif (line[5] == '一级'.decode("utf-8")):
                line[5] = '1'
            elif (line[5] == '二级'.decode("utf-8")):
                line[5] = '2'
            elif (line[5] == '三级'.decode("utf-8")):
                line[5] = '3'
            elif (line[5] == '社区'.decode("utf-8")):
                line[5] = '4'
            else:
                line[5] = '5'
            for i in range(6,20):
            ...
            if(line[21]!="" and line[22]!=""):       ###可能存在未激励出院时间和住院时间的住院记录
                ...
                s = '20' + s[2] + s[1] + s[0]
                t = '20' + t[2] + t[1] + t[0]
                return (line[1],(line[2],line[3],line[5],days,line[10],line[14],line[15],line[17],s,t,line[27]))
            else:
                return (str(999999), 1)
        else:
            return (str(999999),1)
...
def chroricProcessing((key,value)):
    if value[1]==None:
        return (key, (value[0][0], value[0][1], value[0][2],value[0][3],value[0][4],value[0][5],value[0][6],
                      value[0][7], value[0][8], value[0][9],value[0][10],value[0][11],'0',value[0][12],value[0][13],value[0][14],value[0][15]))
    else:
        return (key, (value[0][0], value[0][1], value[0][2], value[0][3], value[0][4], value[0][5], value[0][6],
                      value[0][7], value[0][8], value[0][9],value[0][10], value[0][11],'1',value[0][12],value[0][13],value[0][14],value[0][15]))
...
#####(个人编号,(年度工资，就医序号，医疗机构代码，医院等级 5，住院天数，药品费 10,起付线 14,报销比例 15，统筹账户支付 17,入院日期 21，出院日期，病种
hospitalCharge=data.join(charge)\
    .map(lambda (key,value):(key,(value[1],value[0][0],value[0][1],value[0][2],value[0][3],value
                             [0][4],value[0][5],value[0][6],value[0][7],value[0][8],value[0][9],value[0][10])))\
...
worker=worker.map(lambda line:line.encode("utf-8").split(","))\
    ...
    .sortByKey()
#
# #####(个人编号,(身份,性质，出生日期,性别,年度工资，就医序号，医疗机构代码，医院等级,住院天数,药品费，起付线，报销比例，统筹账户支付，入院日期，出院日期，病种))
# #####(个人编号,(身份,性质，年龄,性别,年度工资，就医序号，医疗机构代码，医院等级,住院天数,药品费，起付线，报销比例，统筹账户支付，入院日期，出院日期，病种)
workerHospitalCharge=hospitalCharge.join(worker) \
    .map(lambda (key,value):(key, (value[1][0], value[1][1],value[1][2], value[1][3],value[0][0], value[0][1],value[0][2],value[0][3],value[0][4],value[0][5],value[0][6],value[0][7],
                                   value[0][8],value[0][9],value[0][10],value[0][11])))\
    .map(ageComputed)\
    .sortByKey()
...
# #####(个人编号,(身份,性质，年龄,性别,年度工资，就医序号，医疗机构代码，医院等级,住院天数,药品费，起付线，报销比例，统筹账户支付，入院日期 21,出院日期 22，病种 26))
# #####(个人编号,(身份,性质，年龄,性别,年度工资，就医序号，医疗机构代码，医院等级,住院天数,药品费，起付线，报销比例，是否患有慢性病,统筹账户支付，入院日期 21,出院日期 22，病种 26))
# #####(就医序号,(身份,性质，年龄,性别,年度工资，医疗机构代码，医院等级,住院天数,药品费，起付线，报销比例，出院病种，是否患有慢性病,统筹账户支付，入院日期 21,出院日期 22，病种 26))
result=workerHospitalCharge.leftOuterJoin(chroric)\
    .map(chroricProcessing) \
    .map(lambda (key, value): (value[5],(key,value[0], value[1], value[2], value[3], value[4],value[6], value[7],
...
result=result.join(fees)\
    .map(lambda(key,value):(value[0][0],(value[0][1], value[0][2], value[0][3], value[0][4],value[0][5],value[0][6], value[0][7],
                                     value[0][8],value[0][9],value[0][10],value[0][11],value[0][12],value[0][14],value[0][15],value[1][0], value[1][1], value[1][2], value[1][3], value[1][4],value[0][13])))\
    .sortByKey()


#####(个人编号,(身份,性质，年龄,性别,年度工资，医疗机构代码，医院等级,住院天数,药品费，起付线，报销比例，是否患有慢性病,入院日期，出院日期，病种，诊疗费、床位费，手术费，护理费、材料费,统筹账户支付))
out = open('output/DetailsOfYZJPTC.csv', 'w+')
