#-*-coding:utf-8-*-
'''
对每个医院每一年住院费用的支出情况,住院人次情况,得出每年各个医院费用支出排名，住院人次排名
'''
...
####（（年份，医院编号),(医院等级，住院总费用，住院统筹费用支出,住院次数)）
hospital=hospital.map(h_precessing)\
    .map(lambda line:(('20'+line[21][-2:],line[3]),(line[5],float(line[6]),float(line[17]),1)))\
...
####（（年份，医院编号),(医院等级，住院总费用，住院统筹费用支出,住院次数,门诊总费用，门诊统筹费用支出,门诊次数)）
result=hospital.leftOuterJoin(menzhen)\
    .map(leftProcessing)\
    .sortByKey() \

###（（年份，医院编号),(医院等级，住院总费用，住院统筹费用支出，住院次数,均次住院总费用,均次统筹费用支出,门诊总费用，门诊统筹费用支出,门诊次数，均次门诊总费用,均次门诊统筹费用支出)）
out=open('output/hospitalByyear.csv','w+')
for (key,value)in result.collect():
    if(value[5]==0):
        out.write("%s,%s,%s,%.2f,%.2f,%d,%.2f,%.2f,%.2f,%.2f,%d,%.2f,%.2f\n" % (
            key[0], key[1], value[0], value[1], value[2], value[3], value[1]/value[3], value[2]/value[3],
